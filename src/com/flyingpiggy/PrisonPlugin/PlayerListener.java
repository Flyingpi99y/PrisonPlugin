package com.flyingpiggy.PrisonPlugin;
...
public class PlayerListener implements Listener{
	public PlayerListener(PrisonPlugin plugin){
	...
	public void entityDamageUsingEnchantedGear(BlockBreakEvent e){
		...
		@EventHandler(priority=EventPriority.LOWEST)
	public void entityDamageUsingEnchantedGear1(BlockBreakEvent g){
		if(g.isCancelled())return;
		Player F = g.getPlayer();
		if(F.getItemInHand()==null|!(F.getItemInHand().getType().name().endsWith("AXE")))return;
		if(!F.getItemInHand().getItemMeta().hasLore()||!F.getItemInHand().hasItemMeta())return;
		if(!hasCustomEnchant(F.getItemInHand().getItemMeta().getLore()))return;
		boolean c = false;
		for(String S : F.getItemInHand().getItemMeta().getLore()){
			if(!isCustomEnchant(S))continue;
			if(S.startsWith(ChatColor.RESET + "Explosive"))c=true;
		}if (!c)return;
		g.getBlock().getWorld().createExplosion((Location) g.getBlock(), 3f);
	}
	...
	public void entityDamageUsingEnchantedGear(EntityShootBowEvent e){
		...
		if(s.startsWith(ChatColor.RESET + "Exploding ")){
			if(p.getInventory().contains(Material.SULPHUR)){
				e.getProjectile().setMetadata("isExploding", new FixedMetadataValue((Plugin) this, true));
				removeItem(p.getInventory(),Material.SULPHUR,1);
			}
		}
		if(s.startsWith(ChatColor.RESET + "Poisonous "))e.getProjectile().setMetadata("poisonous", new FixedMetadataValue( (Plugin) this, level));
		}
	}
	...
	public void onEnchant(EnchantItemEvent e){
		if(e.getItem().getType().name().endsWith("SWORD")){
			...
			if(randomint>=25&&randomint<=99){
			...
			}else if(e.getItem().getType().name().endsWith("AXE")||e.getItem().getType().name().endsWith("SPADE")){
				int randomint = (int)(Math.random()*1000);
				if(randomint<25){
				if(e.getExpLevelCost()==30){
				...
				else if (randomint<900){
					if(e.getExpLevelCost()==30){
						enchantItem("Explosive",1,e.getItem());
						e.getEnchanter().sendMessage(ChatColor.GOLD + "Your tool has become Explosive");
					}
				}
				}
